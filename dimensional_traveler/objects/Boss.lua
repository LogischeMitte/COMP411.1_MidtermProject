...
local function loadSprites()
    if next(sprites) then return end
    sprites[1] = love.graphics.newImage('assets/sprites/bosschill.png')
    sprites[2] = love.graphics.newImage('assets/sprites/bossangry.png')
end
...
function Boss:checkCollision(bullet)
...
function Boss:movePattern(dt, player)
    ...
    if math.abs(player.x - self.x) < 45 and self.isGrounded then
        self.dy = -350
...
function Boss:fireSingle(bullets, player)
...
function Boss:fireSpread(bullets, player)
    ...
    local dir = player.x < self.x and -1 or 1
    local b3 = Bullet.create(halfX, self.y + 4, dir)
    b3.fromPlayer = false
    table.insert(bullets, b3)
end

function Boss:draw()
    local img = sprites[self.phase] or sprites[1]

    local scaleX = self.width  / IMG_W
    local scaleY = self.height / IMG_H
